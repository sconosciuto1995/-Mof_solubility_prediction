...
class SingleGNN(nn.Module):
    """
    GNN that processes a (batch of) graph(s) and returns a pooled representation.
    Designed to be instantiated three times (anion, ligand, solvent).
    """
...
class CombinedModel(nn.Module):
    """
    Model combining three SingleGNN (anion, ligand, solvent) then an NN on the concatenation.
    Returns logits for classification (num_classes).
    """
...
class FeatureExtractor(nn.Module):
    """
    Extracts intermediate representations from the three GNNs without passing through the final NN.
    Useful for analyzing learned features.
    """
...
class ExactGPLayer(gpytorch.models.ExactGP):
    """
    Exact GP model using features extracted by FeatureExtractor.
    """
...
class ExactGNNGP(nn.Module):
    """
    Model combining a GNN feature extractor and an exact GP layer.
    """
